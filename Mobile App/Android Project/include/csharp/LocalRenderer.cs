...
[ObjCRuntime.MonoPInvokeCallback(typeof(OnPipLocationChanged))]
		...
		private static void OnPipLocationChangedDelegate(IntPtr renderer, int x, int y, uint width, uint height, Boolean isExpanded){
			LocalRenderer csRenderer = null;
			if(renderer != IntPtr.Zero){
				if(LocalRenderer.VidyoLocalRendererGetUserDataNative(renderer) == IntPtr.Zero)
					csRenderer = new LocalRenderer(renderer);
				else{
					GCHandle objHandle = (GCHandle)LocalRenderer.VidyoLocalRendererGetUserDataNative(renderer);
					csRenderer = (LocalRenderer)objHandle.Target;
				}
			}
			if(csRenderer._mIRegisterPipEventListener != null)
