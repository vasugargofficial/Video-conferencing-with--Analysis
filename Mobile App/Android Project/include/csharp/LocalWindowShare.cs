...
namespace VidyoClient
{
	public class LocalWindowShare: IDisposable {
		...
		private OnApplicationIconDataUriComplete _mOnApplicationIconDataUriComplete;
		[DllImport(importLib, CallingConvention = CallingConvention.Cdecl)]
		...
		private OnPreviewDataUriComplete _mOnPreviewDataUriComplete;
		[DllImport(importLib, CallingConvention = CallingConvention.Cdecl)]
...
[ObjCRuntime.MonoPInvokeCallback(typeof(OnApplicationIconDataUriComplete))]
		...
		private static void OnApplicationIconDataUriCompleteDelegate(IntPtr w, IntPtr icon){
			LocalWindowShare csW = null;
			if(w != IntPtr.Zero){
				if(LocalWindowShare.VidyoLocalWindowShareGetUserDataNative(w) == IntPtr.Zero)
					csW = new LocalWindowShare(w);
				else{
					GCHandle objHandle = (GCHandle)LocalWindowShare.VidyoLocalWindowShareGetUserDataNative(w);
					csW = (LocalWindowShare)objHandle.Target;
				}
			}
			IntPtr n_icon = VidyoLocalWindowShareOnApplicationIconDataUriCompleteGeticonCStr(icon);

			if(csW._mIGetApplicationIconFrameDataUriAsync != null)
...
[ObjCRuntime.MonoPInvokeCallback(typeof(OnPreviewDataUriComplete))]
		...
		private static void OnPreviewDataUriCompleteDelegate(IntPtr w, IntPtr videoFrame, LocalWindowShareState state){
			...
			if(csW._mIGetPreviewFrameDataUriAsync != null)
