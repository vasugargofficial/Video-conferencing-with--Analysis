...
[ObjCRuntime.MonoPInvokeCallback(typeof(OnPreviewDataUriComplete))]
		...
		private static void OnPreviewDataUriCompleteDelegate(IntPtr m, IntPtr videoFrame, LocalMonitorState state){
			LocalMonitor csM = null;
			if(m != IntPtr.Zero){
				if(LocalMonitor.VidyoLocalMonitorGetUserDataNative(m) == IntPtr.Zero)
					csM = new LocalMonitor(m);
				else{
					GCHandle objHandle = (GCHandle)LocalMonitor.VidyoLocalMonitorGetUserDataNative(m);
					csM = (LocalMonitor)objHandle.Target;
				}
			}
			IntPtr n_videoFrame = VidyoLocalMonitorOnPreviewDataUriCompleteGetvideoFrameCStr(videoFrame);

			if(csM._mIGetPreviewFrameDataUriAsync != null)
